...
class WorldDescription implements IWorld {
    ...
    public int getNumStates() {  
      /**
       * da alle Kombinationen von Feldern mit Pille und ohne theoretisch mˆglich sind
       * vergrˆﬂert sich der Zustandsraum mit der Fakult‰t der Positions-Zust‰nde
       */
    ...
    public int getState() {
      ...
      for (int i = 0; i < pillStates.size(); i++) {
        // jeder Pillenzustand kann als eine Ebene betrachtet werden, die hierarchisch ÔøΩber/unter den anderen liegt
        // zur Vermeidung doppelter ZustÔøΩnde wird dieser abhÔøΩngig von seinem Index dem gloabeln Zustand hinzugefÔøΩgt
        if(!pillStates.get(i).equals(NO_PILL)) {
          state += getPosStates() * (i + 1); // +1 da der nullte Zustand fÔøΩr den Agenten reserviert ist
        }
      }
      return state;
    }
     ...
     * @return Einen hohen Reward, falls der Abstand gro√ü ist, ansonsten einen niedrigen.
     */
    ...
    public void updatePosition(int theAction) {
        ...
        if (isValid(newRow, newCol)) {
            ...
            if(theMap[newRow][newCol] == P4Cm4nEnvironment.WORLD_PILL || 
                ...
                // dabei darf weder die Reihenfolge, noch die GrÔøΩÔøΩe der Liste verÔøΩndert werden (Eindeutigkeit der ZustÔøΩnde!)
